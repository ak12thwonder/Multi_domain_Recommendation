# evaluation/collaborative/book_cf_eval.py
...
def evaluate_book_cf():
    ...
    # Split data: last rating as test
    train = ratings_df.groupby("User-ID").apply(lambda x: x.iloc[:-1]).reset_index(drop=True)
    test = ratings_df.groupby("User-ID").apply(lambda x: x.iloc[-1:]).reset_index(drop=True)

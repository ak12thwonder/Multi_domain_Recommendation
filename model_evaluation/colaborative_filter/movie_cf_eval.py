...
def evaluate_movie_cf():
    ...
    sim_path = os.path.join(cache_dir, "movie_item_similarity_matrix.csv")
    ...
    for _, row in test.iterrows():
        ...
        hit = int(actual_movie in rec_ids)
        hits += hit
        total_users += 1
    ...
    save_accuracy_to_db('movie_cf', round(precision_at_5, 4), total_users, round(recall_at_5, 4), round(f1_at_5, 4), round(mae, 4))
    return pd.DataFrame({
